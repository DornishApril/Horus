...
const { ensureDirectoryExists, resolveAbsolutePath } = require('./utils/pathResolver');
...
class ScreenshotService {
  ...
  async capturePageScreenshot(url, options = {}) {
    ...
    try {
      ...
      const outputDir = resolveAbsolutePath(config.getScreenshotConfig().directory);
      ...
      try {
        const screenshotOptions = {
          ...
          fileName: options.fileName ? 
            `${path.basename(options.fileName, path.extname(options.fileName))}-${index}${path.extname(options.fileName) || '.png'}` : 
            undefined
        };
